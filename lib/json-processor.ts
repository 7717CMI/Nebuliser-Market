...
function checkIfPathHasChildren(
  ...
  try {
    // Path structure: [geography, segmentType, ...segmentPath]
    // Data structure: { geography: { segmentType: { ...segmentPath } } }
    
    // First, try to check allPaths - this is the most reliable method
    // when both value and volume data are uploaded
    if (allPaths && allPaths.length > 0) {
      const childPathsCount = allPaths.filter(otherPath => {
        // Check if otherPath is a child of pathArray
        if (otherPath.path.length <= pathArray.length) {
          return false
        }
        // Check if otherPath starts with pathArray (exact prefix match)
        for (let i = 0; i < pathArray.length; i++) {
          if (otherPath.path[i] !== pathArray[i]) {
            return false
          }
        }
        return true
      ...
      if (childPathsCount > 0) {
        return true
      }
    }
    
    // Fallback: Try to navigate through structure data
    // Path should be: [geography, segmentType, ...segmentPath]
    if (pathArray.length < 2) {
      // Need at least geography and segmentType
      return false
    }
    
    const geography = pathArray[0]
    const segmentType = pathArray[1]
    const segmentPath = pathArray.slice(2)
    
    // Try structure data first
    let current: any = null
    let found = false
    
    if (structureData && structureData[geography] && structureData[geography][segmentType]) {
      current = structureData[geography][segmentType]
      found = true
    ...
    // If not found in structure, try value data
    if (!found && valueData && valueData[geography] && valueData[geography][segmentType]) {
      current = valueData[geography][segmentType]
      found = true
    ...
    // Check if current node has child objects
  ...
  for (let i = 0; i < allPaths.length; i += batchSize) {
    ...
    for (const { path: pathArray, data } of batch) {
      ...
      if (aggregationLevel === null || (isAggregated && aggregationLevel === null)) {
        ...
        // Pass allPaths first as it's the most reliable when both value and volume are uploaded
        const hasChildren = checkIfPathHasChildren(structureData, pathArray, valueData, allPaths)
        ...
        if (isAggregated && !hasChildren && allPaths && allPaths.length > 0) {
          // Double-check using allPaths - this is more reliable when both files are uploaded
          const childPathsCount = allPaths.filter(otherPath => {
            if (otherPath.path.length <= pathArray.length) {
              return false
            }
        ...
        } else if (!hasChildren && volumeData && allPaths && allPaths.length > 0) {
          // Double-check using allPaths - this is more reliable when both files are uploaded
          const childPathsCount = allPaths.filter(otherPath => {
            if (otherPath.path.length <= pathArray.length) {
              return false
