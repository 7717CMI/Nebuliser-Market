...
export function filterData(
  ...
  if (filters.aggregationLevel !== null && filters.aggregationLevel !== undefined) {
    const recordsAtLevel = data.filter(r => r.aggregation_level === filters.aggregationLevel)
    const leafRecordsAtLevel = data.filter(r => r.is_aggregated === false && r.aggregation_level === filters.aggregationLevel)
    console.log(`🔍 Records at level ${filters.aggregationLevel}:`, {
  ...
  const filtered = data.filter((record) => {
    ...
    if (filters.aggregationLevel !== undefined && filters.aggregationLevel !== null) {
      ...
      if (recordLevel === filters.aggregationLevel) {
        ...
        // - etc.
        const hierarchyLevelIndex = filters.aggregationLevel - 1
        
        if (filters.aggregationLevel === 2 && hierarchy.level_1 && hierarchy.level_1.trim() !== '') {
          hasSegmentAtLevel = true
        } else if (filters.aggregationLevel === 3 && hierarchy.level_2 && hierarchy.level_2.trim() !== '') {
          hasSegmentAtLevel = true
        } else if (filters.aggregationLevel === 4 && hierarchy.level_3 && hierarchy.level_3.trim() !== '') {
          hasSegmentAtLevel = true
        } else if (filters.aggregationLevel === 5 && hierarchy.level_4 && hierarchy.level_4.trim() !== '') {
          hasSegmentAtLevel = true
        } else if (filters.aggregationLevel === 6 && hierarchy.level_5 && hierarchy.level_5.trim() !== '') {
          hasSegmentAtLevel = true
        }
